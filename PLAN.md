...
`PxyClaude` is an Anthropic-compatible proxy with optional messaging
workers:
...
Product smoke lives under `smoke/` and is enabled with `PROXYCC_LIVE_SMOKE=1`.
...
explicitly selected by `PROXYCC_SMOKE_PROVIDER_MATRIX`, missing configuration should
