"""Shared filesystem paths for PrxyClaude configuration."""

from pathlib import Path

PRXY_CONFIG_DIRNAME = ".prxy"
PRXY_ENV_FILENAME = ".env"
LEGACY_REPO_DIRNAME = "free-claude-code"
LEGACY_XDG_CONFIG_DIRNAME = ".config"
CLAUDE_WORKSPACE_DIRNAME = "agent_workspace"
PRXY_LOGS_DIRNAME = "logs"
...
def config_dir_path() -> Path:
    ...
    return Path.home() / PRXY_CONFIG_DIRNAME


def managed_env_path() -> Path:
    """Return the default user-managed env file path."""

    return config_dir_path() / PRXY_ENV_FILENAME


def legacy_env_paths() -> tuple[Path, ...]:
    """Return legacy user env paths that can be migrated to ~/.prxy/.env."""

    home = Path.home()
    return (
        home / LEGACY_REPO_DIRNAME / PRXY_ENV_FILENAME,
        home / LEGACY_XDG_CONFIG_DIRNAME / LEGACY_REPO_DIRNAME / PRXY_ENV_FILENAME,
    )
...
def server_log_path() -> Path:
    ...
    return config_dir_path() / PRXY_LOGS_DIRNAME / SERVER_LOG_FILENAME
