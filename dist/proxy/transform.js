...
export function detectTier(modelName) {
    if (OPUS_PATTERNS.test(modelName))
        return "opus";
    if (HAIKU_PATTERNS.test(modelName))
        return "haiku";
    return "sonnet";
}
export function resolveProviderModel(tier, provider) {
...
// ─── Anthropic → OpenAI ───────────────────────────────────────────────────────
export function anthropicToOpenAI(req, targetModel) {
    const messages = [];
    // System prompt
    if (req.system) {
...
export function openAIToAnthropic(oaiResp, originalModel) {
    const choices = oaiResp.choices ?? [];
...
export function translateSSELine(line, originalModel, state) {
