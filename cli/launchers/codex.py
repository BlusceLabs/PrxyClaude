...
_CODEX_AUTH_ENV_KEY = "PRXY_CODEX_API_KEY"
...
def launch(argv: Sequence[str] | None = None) -> None:
    """Launch Codex CLI with PrxyClaude proxy configuration."""

    settings = get_settings()
    proxy_root_url = local_proxy_root_url(settings)
    if error := preflight_proxy(proxy_root_url):
        print(
            f"PrxyClaude proxy is not reachable at {proxy_root_url}: {error}",
            file=sys.stderr,
        )
        print("Start it in another terminal with: prxy-server", file=sys.stderr)
        raise SystemExit(1)
...
def build_codex_launcher_command(
    *,
    ...
    """Return a Codex command with ephemeral PRXY provider config."""
...
def build_codex_launcher_env(
    *,
    ...
    env[_CODEX_AUTH_ENV_KEY] = auth_token.strip() or "prxy-no-auth"
    return env
...
def codex_model_catalog_config_args(
    ...
    try:
        ...
        if not isinstance(models, list) or not models:
            print(
                "PrxyClaude warning: Codex model catalog is empty; "
        ...
        print(
            "PrxyClaude warning: could not prepare Codex model catalog "
...
def codex_config_args(*, api_url: str, model: str | None = None) -> list[str]:
    """Return Codex `-c` assignments for the ephemeral PRXY provider."""

    args = [
        "-c",
        _toml_assignment("model_provider", "prxy"),
        "-c",
        _toml_assignment("model_providers.prxy.name", "PrxyClaude"),
        "-c",
        _toml_assignment("model_providers.prxy.base_url", _ensure_v1_url(api_url)),
        "-c",
        _toml_assignment("model_providers.prxy.env_key", _CODEX_AUTH_ENV_KEY),
        "-c",
        _toml_assignment("model_providers.prxy.wire_api", "responses"),
    ]
    if model:
