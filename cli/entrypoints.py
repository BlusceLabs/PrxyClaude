...
def serve() -> None:
    """Start the FastAPI server (registered as `prxy-server` script)."""
...
def _admin_browser_open_enabled() -> bool:
    """Whether to open /admin when the server becomes reachable (PRXY_OPEN_BROWSER)."""

    raw = os.environ.get("PRXY_OPEN_BROWSER", "true").strip().lower()
...
def _schedule_open_admin_browser(settings: Settings) -> None:
    ...
    threading.Thread(
        target=open_when_ready, name="prxy-open-admin-browser", daemon=True
    ).start()
...
def init() -> None:
    """Scaffold config at ~/.prxy/.env (registered as `prxy-init`)."""
    ...
    if migrated_from is not None:
        ...
        print(
            "Edit it to set your API keys and model preferences, then run: prxy-server"
        )
        return
    ...
    print("Edit it to set your API keys and model preferences, then run: prxy-server")
...
def _migrate_legacy_env_if_missing() -> Path | None:
    ...
    # TODO: Remove after the ~/.prxy/.env migration has had a release cycle.
