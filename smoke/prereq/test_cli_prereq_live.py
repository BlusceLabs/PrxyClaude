...
from smoke.lib.child_process import cmd_proxycc_init, cmd_proxycc_serve
...
def test_proxycc_init_scaffolds_user_config(
...
def test_claude_cli_prompt_when_available(
    ...
    with start_server(
        ...
        result = subprocess.run(
            [claude_bin, "-p", "Reply with exactly PROXYCC_SMOKE_PONG"],
            cwd=tmp_path,
            env=env,
            capture_output=True,
            text=True,
    ...
    if "PROXYCC_SMOKE_PONG" not in result.stdout:
