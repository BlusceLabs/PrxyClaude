...
from smoke.lib.child_process import cmd_prxy_init, cmd_prxyclaude_serve
...
def test_prxy_init_scaffolds_user_config(
    ...
    assert (tmp_path / ".prxy" / ".env").is_file()


def test_prxyclaude_entrypoint_starts_server(smoke_config: SmokeConfig) -> None:
    with start_server(
        smoke_config,
        command=cmd_prxyclaude_serve(),
...
def test_claude_cli_prompt_when_available(
    ...
    with start_server(
        ...
        result = subprocess.run(
            [claude_bin, "-p", "Reply with exactly PRXY_SMOKE_PONG"],
            cwd=tmp_path,
            env=env,
            capture_output=True,
            text=True,
    ...
    if "PRXY_SMOKE_PONG" not in result.stdout:
