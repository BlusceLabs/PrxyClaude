...
The second command skips everything unless `PROXYCC_LIVE_SMOKE=1` is set, but still
writes skip entries to `.smoke-results/`.

## Product Smoke Run

```powershell
$env:PROXYCC_LIVE_SMOKE = "1"
uv run pytest smoke -n 0 -s --tb=short
```
...
catalog/docs and can be overridden with `PROXYCC_SMOKE_MODEL_<PROVIDER>`, for example
`PROXYCC_SMOKE_MODEL_DEEPSEEK=deepseek-v4-pro` (or `deepseek-v4-flash`). If no provider smoke model is
configured, live product smoke fails as `missing_env` unless you explicitly set
`PROXYCC_ALLOW_NO_PROVIDER_SMOKE=1`.
...
| `cli` | `proxycc-init`, server entrypoint, Claude CLI adaptive thinking, session cleanup | Claude CLI binary and provider only for real CLI |
...
| `providers` | multi-turn text, adaptive thinking history, tools, disconnect, errors | configured providers, optional `PROXYCC_SMOKE_MODEL_*` |
...
| `voice` | generated WAV through local Whisper or NVIDIA NIM transcription | `VOICE_NOTE_ENABLED=true`, `PROXYCC_SMOKE_RUN_VOICE=1` |

## Examples

```powershell
$env:PROXYCC_LIVE_SMOKE = "1"
$env:PROXYCC_SMOKE_PROVIDER_MATRIX = "open_router,nvidia_nim,deepseek,lmstudio,llamacpp,ollama"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:PROXYCC_LIVE_SMOKE = "1"
$env:PROXYCC_SMOKE_TARGETS = "ollama"
...
```powershell
$env:PROXYCC_LIVE_SMOKE = "1"
$env:PROXYCC_SMOKE_TARGETS = "telegram,discord,voice"
$env:PROXYCC_SMOKE_RUN_VOICE = "1"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:PROXYCC_LIVE_SMOKE = "1"
$env:PROXYCC_SMOKE_TARGETS = "messaging,config,extensibility"
uv run pytest smoke/product -n 0 -s --tb=short
```

## Environment

- `PROXYCC_ENV_FILE`: explicit dotenv path for startup/config scenarios.
- `PROXYCC_LIVE_SMOKE=1`: enables live smoke execution.
- `PROXYCC_ALLOW_NO_PROVIDER_SMOKE=1`: permits no-provider live smoke for harness work.
- `PROXYCC_SMOKE_TARGETS`: comma-separated targets, or `all`.
- `PROXYCC_SMOKE_PROVIDER_MATRIX`: comma-separated provider prefixes to require.
- `PROXYCC_SMOKE_MODEL_NVIDIA_NIM`, `PROXYCC_SMOKE_MODEL_OPEN_ROUTER`,
  `PROXYCC_SMOKE_MODEL_DEEPSEEK`, `PROXYCC_SMOKE_MODEL_LMSTUDIO`,
  `PROXYCC_SMOKE_MODEL_LLAMACPP`, `PROXYCC_SMOKE_MODEL_OLLAMA`: optional per-provider
  smoke model overrides. Values may include the provider prefix or just the model
  name for that provider.
- `PROXYCC_SMOKE_TIMEOUT_S`: per-request/subprocess timeout, default `45`.
- `PROXYCC_SMOKE_CLAUDE_BIN`: Claude CLI executable name, default `claude`.
- `PROXYCC_SMOKE_TELEGRAM_CHAT_ID`: Telegram chat/user ID for send/edit/delete.
- `PROXYCC_SMOKE_DISCORD_CHANNEL_ID`: Discord channel ID for send/edit/delete.
- `PROXYCC_SMOKE_INTERACTIVE=1`: enables manual inbound Telegram/Discord checks.
- `PROXYCC_SMOKE_RUN_VOICE=1`: allows voice transcription backends to load/run.
...
provider in `PROXYCC_SMOKE_PROVIDER_MATRIX`; selected-but-missing providers fail.
