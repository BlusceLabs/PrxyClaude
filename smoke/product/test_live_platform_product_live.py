...
def test_telegram_live_permissions_e2e(smoke_config: SmokeConfig) -> None:
    ...
    chat_id = os.getenv("PROXYCC_SMOKE_TELEGRAM_CHAT_ID") or (
        smoke_config.settings.allowed_telegram_user_id or ""
    )
    if not chat_id:
        pytest.skip(
            "missing_env: PROXYCC_SMOKE_TELEGRAM_CHAT_ID or "
...
def test_discord_live_permissions_e2e(smoke_config: SmokeConfig) -> None:
    ...
    channel_id = os.getenv("PROXYCC_SMOKE_DISCORD_CHANNEL_ID")
    ...
    if not channel_id:
        pytest.skip(
            "missing_env: PROXYCC_SMOKE_DISCORD_CHANNEL_ID or "
