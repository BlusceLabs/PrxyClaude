...
<div align="center">
  <img src="assets/pic.png" alt="PrxyClaude in action" width="700">
  <p><em>Claude Code running through the PrxyClaude proxy.</em></p>
</div>

<div align="center">
  <img src="assets/codex.png" alt="Codex CLI in action through PrxyClaude" width="700">
  <p><em>Codex CLI using the local PRXY Responses provider.</em></p>
</div>
...
<div align="center">
  ...
  <p><em>Claude Code native <code>/model</code> picker with PRXY gateway models.</em></p>
</div>

<div align="center">
  <img src="assets/codex-model-picker.png" alt="Codex model picker showing generated PRXY model catalog" width="700">
  <p><em>Codex native <code>/model</code> picker with the generated PRXY catalog.</em></p>
</div>

## Star History

<div align="center">
  <a href="https://star-history.com/#BlusceLabs/PrxyClaude&Date">
    <picture>
      <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=BlusceLabs/PrxyClaude&type=Date&theme=dark">
      <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=BlusceLabs/PrxyClaude&type=Date">
      <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=BlusceLabs/PrxyClaude&type=Date" width="700">
    </picture>
  </a>
</div>
...
- `prxy-claude` and `prxy-codex` launchers that read the current Admin UI port and auth token each time they start.
...
- Native Codex `/model` picker support when launched through `prxy-codex`, using a generated local model catalog.
...
```bash
curl -fsSL "https://github.com/BlusceLabs/PrxyClaude/blob/main/scripts/install.sh?raw=1" | sh
```

Windows PowerShell:

```powershell
irm "https://github.com/BlusceLabs/PrxyClaude/blob/main/scripts/install.ps1?raw=1" | iex
```

Review the installers at [scripts/install.sh](https://github.com/BlusceLabs/PrxyClaude/blob/main/scripts/install.sh) and [scripts/install.ps1](https://github.com/BlusceLabs/PrxyClaude/blob/main/scripts/install.ps1). They install Claude Code and Codex when missing, then install or update the proxy. Re-run these commands to update to the latest version.

To remove only PrxyClaude (not uv, Claude Code, Codex, or the uv-managed Python runtime):

macOS/Linux:

```bash
curl -fsSL "https://raw.githubusercontent.com/BlusceLabs/PrxyClaude/main/scripts/uninstall.sh" | sh
```

Windows PowerShell:

```powershell
irm "https://raw.githubusercontent.com/BlusceLabs/PrxyClaude/main/scripts/uninstall.ps1" | iex
```

Review [scripts/uninstall.sh](https://github.com/BlusceLabs/PrxyClaude/blob/main/scripts/uninstall.sh) and [scripts/uninstall.ps1](https://github.com/BlusceLabs/PrxyClaude/blob/main/scripts/uninstall.ps1). They remove the PRXY uv tool and always delete `~/.prxy/`. Stop any running `prxy-server`, `prxy-claude`, `prxy-codex`, `prxy-init`, or `prxyclaude` process before uninstalling.
...
Keep `prxy-server` running while you work.
...
`prxy-claude` reads the current configured port and auth token each time it starts, sets the Claude Code environment variables (including a 190k-token `CLAUDE_CODE_AUTO_COMPACT_WINDOW` for auto-compaction), and then launches the real `claude` command. When proxy auth is disabled, it still passes `ANTHROPIC_AUTH_TOKEN=prxy-no-auth` so newer Claude Code versions do not stop at their local login gate before contacting the proxy.
...
`prxy-codex` reads the same port and auth token, registers an ephemeral `prxy` model provider that points at the local proxy's `/v1/responses` endpoint, generates a Codex model catalog from the proxy's `/v1/models` response, sets `PRXY_CODEX_API_KEY` from the Admin UI auth token, strips official `OPENAI_*` credentials from the child environment, and then launches the real `codex` command. Type `/model` inside Codex to open its native picker. Pass through Codex args as usual, for example `prxy-codex exec "hello"`.
...
The Admin UI manages proxy config, restarts the server when runtime settings change, and `prxy-claude` reads the current Admin UI-managed port and auth token every time it starts. It also sets `CLAUDE_CODE_AUTO_COMPACT_WINDOW` to `190000` for auto-compaction. When proxy auth is blank, `prxy-claude` injects `ANTHROPIC_AUTH_TOKEN=prxy-no-auth` only to satisfy Claude Code's local login check; the proxy still treats blank auth as disabled.
...
The installer provisions Codex when it is missing (`npm install -g @openai/codex`). `prxy-codex` injects ephemeral Codex config on every launch:

- `model_provider=prxy`
- `model_providers.prxy.base_url=http://127.0.0.1:<PORT>/v1`
- `model_providers.prxy.env_key=PRXY_CODEX_API_KEY`
- `model_providers.prxy.wire_api=responses`
- `model_catalog_json=~/.prxy/codex-model-catalog.json`

The Admin UI auth token is reused as `PRXY_CODEX_API_KEY`. Official OpenAI credentials are stripped from the child environment so traffic stays on the local proxy. The generated model catalog lets Codex's native `/model` picker list provider-selectable PRXY model slugs. If the catalog cannot be fetched or written, `prxy-codex` warns and still launches without picker injection.
...
```bash
codex \
  -c 'model_provider="prxy"' \
  -c 'model_providers.prxy.name="PrxyClaude"' \
  -c 'model_providers.prxy.base_url="http://127.0.0.1:8082/v1"' \
  -c 'model_providers.prxy.env_key="PRXY_CODEX_API_KEY"' \
  -c 'model_providers.prxy.wire_api="responses"' \
  exec "hello"
```

Set `PRXY_CODEX_API_KEY` to the same value as `ANTHROPIC_AUTH_TOKEN` in the Admin UI.
...
Create or edit that file with the `prxy` provider pointing at your local proxy:

```toml
model_provider = "prxy"
model = "nvidia_nim/nvidia/nemotron-3-super-120b-a12b"

[model_providers.prxy]
name = "PrxyClaude"
base_url = "http://127.0.0.1:8082/v1"
env_key = "PRXY_CODEX_API_KEY"
wire_api = "responses"
```
...
The bot wrapper runs Claude Code sessions remotely, streams progress, supports reply-based conversation branches, and can stop or clear tasks. Discord and Telegram bots use Claude Code today; use `prxy-codex` or the Codex VS Code extension for Codex sessions.
...
1. With `prxy-server` running, open the **Admin UI** URL from the terminal output.
...
Voice notes work on Discord and Telegram after you extend your [PrxyClaude install](#1-fast-install) with the matching optional extras.

macOS/Linux:

```bash
# NVIDIA NIM transcription (Riva gRPC)
curl -fsSL "https://github.com/Alishahryar1/prxyclaude/blob/main/scripts/install.sh?raw=1" | sh -s -- --voice-nim

# Local Whisper (CPU or CUDA)
curl -fsSL "https://github.com/Alishahryar1/prxyclaude/blob/main/scripts/install.sh?raw=1" | sh -s -- --voice-local

# Both backends
curl -fsSL "https://github.com/Alishahryar1/prxyclaude/blob/main/scripts/install.sh?raw=1" | sh -s -- --voice-all

# Local Whisper with CUDA
curl -fsSL "https://github.com/Alishahryar1/prxyclaude/blob/main/scripts/install.sh?raw=1" | sh -s -- --voice-local --torch-backend cu130
```
...
```powershell
# NVIDIA NIM transcription (Riva gRPC)
& ([scriptblock]::Create((irm "https://github.com/Alishahryar1/prxyclaude/blob/main/scripts/install.ps1?raw=1"))) -VoiceNim

# Local Whisper (CPU or CUDA)
& ([scriptblock]::Create((irm "https://github.com/Alishahryar1/prxyclaude/blob/main/scripts/install.ps1?raw=1"))) -VoiceLocal

# Both backends
& ([scriptblock]::Create((irm "https://github.com/Alishahryar1/prxyclaude/blob/main/scripts/install.ps1?raw=1"))) -VoiceAll

# Local Whisper with CUDA
& ([scriptblock]::Create((irm "https://github.com/Alishahryar1/prxyclaude/blob/main/scripts/install.ps1?raw=1"))) -VoiceLocal -TorchBackend cu130
```

Restart `prxy-server` after reinstalling.
...
<div align="center">
  <img src="assets/how-it-works.svg" alt="PrxyClaude request flow architecture" width="900">
</div>
...
- `prxy-codex` registers a custom `prxy` provider that points Codex at the local proxy's `/v1/responses` endpoint.
...
```bash
git clone https://github.com/Alishahryar1/prxyclaude.git
cd prxyclaude
...
- `prxy-server`: starts the proxy with configured host and port.
- `prxy-init`: optional advanced scaffold for `~/.prxy/.env`; prefer the **Admin UI** for normal configuration.
- `prxy-claude`: launches Claude Code with the configured local proxy URL, an auth-token env var or `prxy-no-auth` sentinel, model discovery flag, and a 190k `CLAUDE_CODE_AUTO_COMPACT_WINDOW` for auto-compaction.
- `prxy-codex`: launches Codex with ephemeral `prxy` provider config pointing at the local proxy's `/v1/responses` endpoint, a generated native `/model` picker catalog, and `PRXY_CODEX_API_KEY` from the Admin UI auth token.
- `prxyclaude`: compatibility alias for `prxy-server`.
...
- Report bugs and feature requests in [Issues](https://github.com/Alishahryar1/prxyclaude/issues). For bug always include all model mapping, current model when issue occured and the issue string
