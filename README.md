...
[![Tested with Pytest](https://img.shields.io/badge/testing-Pytest-00c0ff.svg?style=for-the-badge)](https://github.com/BlusceLabs/prxyclaude/actions)
...
<div align="center">
  <img src="pic.png" alt="PrxyClaude in action" width="700">
...
```bash
ANTHROPIC_AUTH_TOKEN="prxyclaude" ANTHROPIC_BASE_URL="http://localhost:8082" claude
```
...
"claude-code.environmentVariables": [
  ...
  { "name": "ANTHROPIC_AUTH_TOKEN", "value": "prxyclaude" }
]
```
...
`prxyclaude-pick` is an interactive model selector that lets you choose any model from your active provider each time you launch Claude, without editing `MODEL` in `.env`.
...
alias prxyclaude-pick="/absolute/path/to/prxyclaude/prxyclaude-pick"
```

Then reload your shell (`source ~/.zshrc` or `source ~/.bashrc`) and run `prxyclaude-pick` to pick a model and launch Claude.

**Skip the picker with a fixed model** (no picker needed):

```bash
alias claude-kimi='ANTHROPIC_BASE_URL="http://localhost:8082" ANTHROPIC_AUTH_TOKEN="prxyclaude:moonshotai/kimi-k2.5" claude'
```
...
```
┌─────────────────┐        ┌──────────────────────┐        ┌──────────────────┐
│  Claude Code    │───────>│     PrxyClaude       │───────>│  LLM Provider    │
...
- Report bugs or suggest features via [Issues](https://github.com/BlusceLabs/prxyclaude/issues)
