...
impl DeepSeekProvider {
    ...
    fn build_request_url(&self) -> String {
        self.base_url.as_deref().unwrap_or("https://api.deepseek.com").to_string() + "/v1/chat/completions"
    }
...
impl Provider for DeepSeekProvider {
    ...
    async fn list_models(&self) -> Result<Vec<String>, ProviderError> {
        Ok(vec!["deepseek-chat".to_string(), "deepseek-coder".to_string()])
    }
...
impl DeepSeekProvider {
    ...
    fn convert_from_openai_format(&self, response: &OpenAIResponse) -> Result<crate::models::MessagesResponse, ProviderError> {
        let usage = response.usage.clone().unwrap_or_else(|| Usage {
            input_tokens: 0,
            output_tokens: 0,
