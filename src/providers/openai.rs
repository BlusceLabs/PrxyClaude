...
impl OpenAIProvider {
    pub fn new(api_key: Option<String>, base_url: Option<String>, organization: Option<String>) -> Self {
        Self {
            api_key,
            base_url,
            organization,
    ...
    fn build_request_url(&self) -> String {
        self.base_url.as_deref().unwrap_or("https://api.openai.com/v1").to_string() + "/chat/completions"
    }
...
impl Provider for OpenAIProvider {
    ...
    async fn list_models(&self) -> Result<Vec<String>, ProviderError> {
        let client = reqwest::Client::new();
        let url = self.base_url.as_deref().unwrap_or("https://api.openai.com/v1").to_string() + "/models";
        let headers = self.build_request_headers();
