...
function authMiddleware(req: Request, res: Response, next: NextFunction): void {
  ...
  const authToken = req.headers["x-api-key"] as string | undefined;
...
function adminAuthMiddleware(req: Request, res: Response, next: NextFunction): void {
  const cfg = getConfig();
  const token = (req.headers["x-admin-token"] as string | undefined) ?? req.query.token;
...
app.post("/v1/messages", authMiddleware, async (req: Request, res: Response): Promise<void> => {
