...
fn test_config_creation() {
    ...
    assert_eq!(config.server.addr, "0.0.0.0:8082");
    assert_eq!(config.providers.default_provider, "open_router");
}

#[test]
fn test_message_creation() {
    let message = Message::new(Role::User, ContentOrBlocks::String("Hello, world!".to_string()));
...
fn test_messages_request_creation() {
    let messages = vec![Message::new(Role::User, ContentOrBlocks::String("Hello!".to_string()))];
