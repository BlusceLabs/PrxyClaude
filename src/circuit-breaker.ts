...
function getCircuit(providerId: string): CircuitBreakerState {
...
export function recordFailure(providerId: string, reason?: string): void {
  const cb = getCircuit(providerId);
  const cfg = getConfig().circuitBreaker;
  ...
  if (cb.state === "half-open" || cb.failures >= cfg.failureThreshold) {
    ...
    log(
      "error",
      `[circuit] ${providerId} → OPEN (retry in ${cfg.halfOpenAfterMs / 1000}s)`
    );
  }
}
