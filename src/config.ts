...
import type { PrxyConfig, ProviderConfig } from "./types.js";

const CONFIG_PATH = process.env.CONFIG_PATH ?? path.join(process.cwd(), "prxyclaude.json");

// ─── Defaults ────────────────────────────────────────────────────────────────

const DEFAULT_CONFIG: PrxyConfig = {
  ...
  logging: {
    level: (process.env.LOG_LEVEL as PrxyConfig["logging"]["level"]) ?? "info",
    requests: true,
  },
...
function buildProvidersFromEnv(): ProviderConfig[] {
  ...
  // ── Together AI ──
  const togetherKeys = [process.env.TOGETHER_API_KEY].filter(Boolean) as string[];
  ...
  const anthropicKeys = [process.env.ANTHROPIC_API_KEY].filter(Boolean) as string[];
...
function loadConfig(): PrxyConfig {
  let fileConfig: Partial<PrxyConfig> = {};
  ...
  const providers =
    fileConfig.providers && fileConfig.providers.length > 0
...
export function saveConfig(cfg: PrxyConfig): void {
  fs.writeFileSync(CONFIG_PATH, JSON.stringify(cfg, null, 2));
}

// Singleton
let _config: PrxyConfig | null = null;

export function getConfig(): PrxyConfig {
  if (!_config) _config = loadConfig();
  return _config;
}

export function reloadConfig(): PrxyConfig {
  _config = loadConfig();
  return _config;
}

export function patchConfig(patch: Partial<PrxyConfig>): PrxyConfig {
