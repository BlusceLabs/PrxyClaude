...
function cacheKey(req: AnthropicRequest): string {
...
export function cacheGet(req: AnthropicRequest): unknown | null {
  const cfg = getConfig().cache;
  if (!cfg.enabled || req.stream) return null; // never cache streams
...
export function cacheSet(req: AnthropicRequest, value: unknown): void {
  const cfg = getConfig().cache;
  if (!cfg.enabled || req.stream) return;
