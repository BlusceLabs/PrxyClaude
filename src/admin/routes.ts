...
export function adminRouter(): Router {
  ...
  router.post("/provider/:id/enable", (req: Request, res: Response) => {
    const cfg = getConfig();
  ...
  router.post("/provider/:id/disable", (req: Request, res: Response) => {
    const cfg = getConfig();
    const p = cfg.providers.find((p) => p.id === req.params.id);
    if (!p) return res.status(404).json({ error: "provider not found" });
    p.enabled = false;
  ...
  router.post("/provider/:id/reset-circuit", (req: Request, res: Response) => {
  ...
  router.put("/provider/:id/priority", (req: Request, res: Response) => {
    const cfg = getConfig();
    const p = cfg.providers.find((p) => p.id === req.params.id);
    if (!p) return res.status(404).json({ error: "provider not found" });
    const priority = Number(req.body.priority);
    if (isNaN(priority)) return res.status(400).json({ error: "invalid priority" });
    p.priority = priority;
    patchConfig(cfg);
