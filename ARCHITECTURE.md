...
This document is a maintainer-oriented map of PrxyClaude. It explains the
...
PrxyClaude is a local proxy for agent clients. It accepts Anthropic
...
PRXY optimizes for installed user workflows, not internal compatibility. The
...
- `prxy-server` and the local Admin UI for configuring supported providers,
  model routing, auth, server tools, messaging, and diagnostics.
- `prxy-claude`, Claude Code, and the Anthropic-compatible proxy behavior Claude
...
- `prxy-codex`, Codex CLI/extensions, and the streaming OpenAI Responses behavior
...
- `prxy-server` and `prxyclaude` call `cli.entrypoints:serve`.
- `prxy-init` calls `cli.entrypoints:init`.
- `prxy-claude` calls `cli.launchers.claude:launch`.
- `prxy-codex` calls `cli.launchers.codex:launch`.
...
and [scripts/uninstall.ps1](scripts/uninstall.ps1) remove only the PRXY uv tool and always
delete the managed `~/.prxy/` tree from [config/paths.py](config/paths.py); they do not remove
...
1. repo-local `.env`;
2. managed `~/.prxy/.env`;
3. optional `PRXY_ENV_FILE`, appended when present.
...
- config directory: `~/.prxy`;
- managed env file: `~/.prxy/.env`;
- generated Codex model catalog: `~/.prxy/codex-model-catalog.json`;
- agent workspace: `~/.prxy/agent_workspace`;
- server log: `~/.prxy/logs/server.log`.
...
Codex-specific model picker shaping stays out of this route. `prxy-codex` fetches
the same `/v1/models` response at launch, converts PRXY gateway IDs into
provider-selectable Codex slugs, writes `~/.prxy/codex-model-catalog.json`, and
passes it as `model_catalog_json`. Codex users open the native picker with
`/model`; PRXY does not implement a proxy-level `/models` alias.
...
- streaming-only `/v1/responses` support for Codex/PRXY workflows;
...
PRXY accepts omitted `stream` or `stream: true`; `stream: false` is rejected with
an OpenAI-shaped client error because installed PRXY/Codex workflows only need
...
PRXY does not validate custom-tool grammars.
...
`encrypted_content`; the opaque value is not exposed as visible text and PRXY
...
`prxy-claude` launcher:

- `prxy-claude` strips inherited `ANTHROPIC_*` variables, sets
  ...
  becomes the local-only `prxy-no-auth` sentinel so Claude Code reaches the proxy
  instead of stopping at its login gate.

[cli/launchers/codex.py](cli/launchers/codex.py) owns the installed
`prxy-codex` launcher:

- `prxy-codex` strips official OpenAI and Codex credential variables.
- It creates an ephemeral `prxy` model provider with `wire_api = "responses"` and
  a base URL pointing at the local proxy `/v1` path.
- After proxy health succeeds, it fetches `/v1/models`, writes a generated Codex
  `model_catalog_json` file under `~/.prxy/`, and injects that path so Codex's
  native `/model` picker lists PRXY provider slugs. Catalog generation is
  fail-open: launch continues with a warning if the catalog cannot be prepared.
- It stores the proxy auth token in `PRXY_CODEX_API_KEY` for Codex to read.
...
Codex is supported through `prxy-codex` and Codex extensions. PRXY does not keep an
