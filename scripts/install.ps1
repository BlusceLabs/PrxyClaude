...
$RepoGitUrl = "git+https://github.com/BlusceLabs/PrxyClaude.git"
...
Installs Claude Code and Codex if missing, installs or updates uv, Python 3.14.0, and PrxyClaude.
...
function Get-PackageSpec {
    ...
    if ($includeNim -and $includeLocal) {
        return "prxyclaude[voice,voice_local] @ $RepoGitUrl"
    }

    if ($includeNim) {
        return "prxyclaude[voice] @ $RepoGitUrl"
    }

    if ($includeLocal) {
        return "prxyclaude[voice_local] @ $RepoGitUrl"
    }

    return $RepoGitUrl
}

function Install-OrUpdatePrxyClaude {
...
Write-Step "Installing or updating PrxyClaude"
Install-OrUpdatePrxyClaude

Write-Host ""
Write-Host "PrxyClaude is installed. Start the proxy with: prxy-server"
Write-Host "Run Claude Code with: prxy-claude"
Write-Host "Run Codex with: prxy-codex"
