...
REPO_GIT_URL="git+https://github.com/BlusceLabs/PrxyClaude.git"
...
package_spec() {
    ...
    if [ "$include_nim" -eq 1 ] && [ "$include_local" -eq 1 ]; then
        printf 'prxyclaude[voice,voice_local] @ %s' "$REPO_GIT_URL"
    elif [ "$include_nim" -eq 1 ]; then
        printf 'prxyclaude[voice] @ %s' "$REPO_GIT_URL"
    elif [ "$include_local" -eq 1 ]; then
        printf 'prxyclaude[voice_local] @ %s' "$REPO_GIT_URL"
    else
...
step "Installing or updating PrxyClaude"
install_prxyclaude

printf '\nPrxyClaude is installed. Start the proxy with: prxy-server\n'
printf 'Run Claude Code with: prxy-claude\n'
printf 'Run Codex with: prxy-codex\n'
