...
def _run_init(tmp_home: Path) -> tuple[str, Path]:
    ...
    env_file = tmp_home / ".config" / "PxyClaude" / ".env"
    printed: list[str] = []
...
def test_init_creates_parent_directories(tmp_path: Path) -> None:
    """init() creates ~/.config/PxyClaude/ even if it doesn't exist."""
    config_dir = tmp_path / ".config" / "PxyClaude"
...
def test_init_skips_if_env_already_exists(tmp_path: Path) -> None:
    ...
    env_file = tmp_path / ".config" / "PxyClaude" / ".env"
...
def test_init_prints_next_step_hint(tmp_path: Path) -> None:
    """init() tells the user to run PxyClaude after editing .env."""
    output, _ = _run_init(tmp_path)

    assert "PxyClaude" in output
