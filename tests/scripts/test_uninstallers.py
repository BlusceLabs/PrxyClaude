...
def test_uninstall_sh_removes_uv_tool_and_purges_prxy_home() -> None:
    text = _script_text("uninstall.sh")
    tool_body = _braced_body(text, "uninstall_prxyclaude()")
    purge_body = _braced_body(text, "purge_prxy_home()")

    assert "Does not remove uv, Claude Code, Codex" in text
    assert "uv tool uninstall" in tool_body
    assert 'PACKAGE_NAME="prxyclaude"' in text
...
def test_uninstall_sh_fails_when_fcc_commands_are_running() -> None:
    ...
    assert "PRXY_COMMANDS" in text

    assert "PrxyClaude is still running" in guard_body
    assert (
        'step "Checking for running PrxyClaude processes"\nassert_no_prxy_processes_running'
        in main
    )
...
def test_uninstall_ps1_removes_uv_tool_and_purges_prxy_home() -> None:
    ...
    assert "Does not remove uv, Claude Code, Codex" in text
    assert "uv tool uninstall" in tool_body
    assert '$PackageName = "prxyclaude"' in text
    assert "uv not found on PATH; skipping uv tool uninstall." in tool_body
...
def test_uninstall_sh_generic_uv_failure_does_not_delete_prxy_home(
    tmp_path: Path,
) -> None:
    sh = shutil.which("sh")
    if sh is None:
    ...
    assert prxy_home.exists()
    assert "failed with exit code 42" in result.stderr
    assert "before deleting ~/.prxy" in result.stderr


def test_uninstall_sh_missing_tool_still_deletes_prxy_home(tmp_path: Path) -> None:
    sh = shutil.which("sh")
    if sh is None:
        pytest.skip("sh is not available on this platform")
    ...
    uv = bin_dir / "uv"
    uv.write_text(
        "#!/bin/sh\nprintf '%s\\n' 'tool prxyclaude is not installed' >&2\nexit 2\n",
        encoding="utf-8",
    )
    uv.chmod(uv.stat().st_mode | stat.S_IXUSR)
    ...
    assert result.returncode == 0
    assert not prxy_home.exists()


def test_uninstall_sh_missing_uv_still_deletes_prxy_home(tmp_path: Path) -> None:
    sh = shutil.which("sh")
    if sh is None:
        pytest.skip("sh is not available on this platform")
...
def test_uninstall_ps1_generic_uv_failure_does_not_delete_prxy_home(
    tmp_path: Path,
) -> None:
...
def test_uninstall_ps1_missing_tool_still_deletes_prxy_home(tmp_path: Path) -> None:
    pwsh = shutil.which("pwsh") or shutil.which("powershell")
    if pwsh is None:
        pytest.skip("PowerShell is not available on this platform")
    ...
    _write_mock_uv(
        bin_dir,
        message="tool prxyclaude is not installed",
        exit_code=2,
    )
...
def test_uninstall_ps1_missing_uv_still_deletes_prxy_home(tmp_path: Path) -> None:
    pwsh = shutil.which("pwsh") or shutil.which("powershell")
    if pwsh is None:
        pytest.skip("PowerShell is not available on this platform")
